...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if warning == True:
            addFile('[COLOR dodgerblue][B]ACTUALIZAR PARCHE[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def freshStart(install=None, over=False):
    ...
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Kodi se restaurara a su configuracion inicial" % COLOR2, "eliminando todos los archivos antes de", "Instalar la Build Inicial [COLOR %s]%s[/COLOR]" % (COLOR1, install), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Continuar[/COLOR][/B]')
